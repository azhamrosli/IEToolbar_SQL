            ...
            try
            {
                ...
                switch (strPage)
                {
                    case "BE2016Page1": case "BE2016Page2": case "BE2016Page3": 
                        ...
                        //weihong 
                        strQuery = "select convert(nvarchar(20), TP_WORKER_APPROVEDATE, 103), TP_COM_ADD_STATUS, convert(nvarchar(20), TP_DOB, 103), convert(nvarchar(20), TP_HW_DOB, 103),TP_BUSINESS_ECOMMERCE,TP_JKDM,TP_DISPOSAL1976,TP_LDMN_DISPOSAL,TP_GST,TP_TEL_KOD,TP_MOBILE_KOD,TP_FAX_KOD,TP_MOTHER_IC,TP_FATHER_IC,TP_BWA,TP_DOB from TAXP_PROFILE2 where TP_REF_NO=?";
                        cmdOdbc = new OdbcCommand(strQuery, connOdbc);
                        cmdOdbc.Parameters.Add(new OdbcParameter("@taxpayer", strTaxPayer.ToString()));
                        daOdbc = new OdbcDataAdapter(cmdOdbc);
                    ...
                    //break;
                        ...
                        daOdbc.Fill(dtTemp);
                        dsData.Tables.Add(dtTemp);
                        //daOdbc.Dispose();P4_TAX_RELIEF_CHILD_18ABOVE_1000
                        cmdOdbc.Dispose();
                        ...
                        strQuery = "select TCC_KEY, TCC_100, TCC_50 " +
                        "FROM TAX_RELIEF_CHILD WHERE TC_KEY IN (SELECT TC_KEY FROM TAX_COMPUTATION WHERE TC_REF_NO=? and TC_YA=?) AND TCC_KEY = 16 AND TCC_RELIEF = '7,000' and TCC_50 = '7,000'";
                        cmdOdbc = new OdbcCommand(strQuery, connOdbc);
                        cmdOdbc.Parameters.Add(new OdbcParameter("@taxpayer", strTaxPayer));
                        cmdOdbc.Parameters.Add(new OdbcParameter("@ya", strYA));
                        daOdbc = new OdbcDataAdapter(cmdOdbc);
